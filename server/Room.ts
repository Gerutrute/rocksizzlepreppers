...
import type { NetworkCore, NetworkPlayer, RoomSnapshot, Team } from '../src/game-core/protocol'
...
type PlayerState=NetworkPlayer&{input:InputState;dashReady:number;nextAction:number}
type CoreState=NetworkCore&{fuse:number;chain:number}

export class Room{
  readonly players=new Map<string,PlayerState>()
  readonly cores=new Map<string,CoreState>()
  readonly events:Array<{event:string;actorId?:string;targetId?:string;coreId?:string;x?:number;z?:number;team?:Team;chain?:number}>=[]
  ...
  constructor(readonly id:string,now=Date.now()){this.startedAt=now;this.lastHumanJoin=now}

  join(id:string,name:string,now=Date.now()){
    if(![...this.players.values()].some(player=>!player.bot)){
      this.players.clear();this.cores.clear();this.events.length=0;this.tick=0;this.startedAt=now;this.winner=null;this.lastFanPush=0;this.lastVehicleCell=-99;this.startedEventSent=false;this.resetArena()
    }
    ...
    const player:PlayerState={id,name:name.slice(0,16)||`RIPPLE-${slot+1}`,slot,bot:false,team,x:spawn.x,z:spawn.z,yaw,hits:0,downedUntil:0,eliminated:false,lastInput:0,input:{dx:0,dz:0,seq:0},dashReady:0,nextAction:0}
  ...
  leave(id:string){
    ...
    if(![...this.players.values()].some(player=>!player.bot)){this.players.clear();this.cores.clear();this.events.length=0;this.tick=0;this.startedAt=Date.now();this.winner=null;this.lastHumanJoin=Date.now();this.startedEventSent=false;this.lastVehicleCell=-99;this.resetArena()}
  }
  ...
  action(id:string,action:'PLACE'|'DASH'|'KICK'|'THROW'|'RESCUE',direction:{x:number;z:number},now=Date.now()){
  ...
  rematch(id:string,now=Date.now()){
    ...
    this.cores.clear();this.winner=null;this.startedAt=now;this.tick=0;this.lastFanPush=0;this.lastVehicleCell=-99;this.lastHumanJoin=now;this.startedEventSent=false;this.resetArena()
    for(const player of this.players.values()){
      const spawn=GIANT_PLAYROOM.spawnPoints[player.slot]
      player.x=spawn.x;player.z=spawn.z;player.yaw=player.team==='cyan'?Math.PI/2:-Math.PI/2;player.hits=0;player.downedUntil=0;player.eliminated=false;player.dashReady=0;player.nextAction=now+800+player.slot*350
  ...
  private canOccupy(player:PlayerState,x:number,z:number){
    if(isCellBlocked(this.arena,worldToGrid({x,z})))return false
  ...
  private fillBots(now:number){
    ...
    for(let slot=0;slot<4;slot++){
      ...
      this.players.set(id,{id,name:names[slot],slot,bot:true,team,x:spawn.x,z:spawn.z,yaw,hits:0,downedUntil:0,eliminated:false,lastInput:0,input:{dx:0,dz:0,seq:0},dashReady:0,nextAction:now+800+slot*350})
  ...
  private placeCore(player:PlayerState){
    ...
    if(active>=GAME_BALANCE.CORE_CAPACITY||isCellBlocked(this.arena,cell)||this.coreAt(cell.x,cell.z))return false
    const id=`${this.id}-${++this.coreId}`;this.cores.set(id,{id,owner:player.id,team:player.team,x:cell.x,z:cell.z,fuse:GAME_BALANCE.CORE_FUSE_SECONDS,chain:1});this.events.push({event:'CORE_PLACED',actorId:player.id,coreId:id});return true
  }

  private explode(core:CoreState,now:number){
    if(!this.cores.has(core.id))return
    const wallHits=blastHitWalls(this.arena,core,GAME_BALANCE.CORE_RANGE)
    const cells=traceExplosion(this.arena,core,GAME_BALANCE.CORE_RANGE),keys=new Set(cells.map(cellKey));this.cores.delete(core.id);this.events.push({event:'CORE_EXPLODED',actorId:core.owner,coreId:core.id,x:core.x,z:core.z,team:core.team,chain:core.chain})
    for(const wall of wallHits){const key=cellKey(wall);this.arena.walls.delete(key);this.destroyedWalls.add(key);this.events.push({event:'OBJECT_DESTROYED',x:wall.x,z:wall.z,team:core.team})}
    for(const player of this.players.values()){
  ...
  private resetArena(){this.arena.walls.clear();for(const wall of GIANT_PLAYROOM.walls)this.arena.walls.add(wall);this.destroyedWalls.clear()}
}
